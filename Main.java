...
public class Main extends Application {
    ...
    int SaveCountTime=0;
    int SaveCountTimeDelta=1;
    String fileName = "objectsRepeat";
     ...
     */ public void start(Stage primaryStage) throws Exception {
        ...
        Button btnGenerateRepeats = new Button("Generate repeats");

        difficulty.getChildren()
            .addAll(btnDifEasy, btnDifNormal, btnDifHard, btnBack, btnAuto, btnRepeat,btnGenerateRepeats);
        ...
        btnGenerateRepeats.setOnAction(e ->{
        ...
        btnGenerateRepeats.setTranslateX(-80);
        btnGenerateRepeats.setTranslateY(80);
        ...
        gameLoop = new AnimationTimer() {
            @Override public void handle(long now) {
                if (Settings.repeat) {
                    ...
                    try {
                        gameState = loadRepeat(fileName,gameState);
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                    ...
                    ThreadForSave SaveThread =
                        new ThreadForSave(fileName,players, enemies, bullets, player1bullets, player2bullets,
                            playerMissileList);
                    SaveThread.setDaemon(true);
                    SaveCountTime+=SaveCountTimeDelta;
                    if (SaveCountTime==3) {
                        SaveCountTime=0;
                        SaveThread.start();
                        try {
                    ...
                    if (Settings.enemy - Settings.enemyTotal + enemies.size()==0) {
                        BtnMenuAction(primaryStage);
                    }
                }
            }
    ...
    private void BtnMenuAction(Stage primaryStage)
    {
    ...
    class ThreadForSave extends Thread {
        private List<Player> players;
        private List<EnemyTank> enemies;
        private List<Bullet> player1bullets;
        private List<Bullet> player2bullets;
        private List<Bullet> bullets;
        private List<Missile> playerMissileList;
        private int CountTime=0;
        private int CountTimeDelta=1;
        private String fileName;

        public ThreadForSave(String fileName, List<Player> player, List<EnemyTank> enemy, List<Bullet> bullets,
            ...
            this.playerMissileList = missiles;
        ...
        public void run() {
                if (Settings.saveFirstTime) {
                    ...
                    Utils.saveGame(fileName, this.players, this.enemies, this.bullets,
                        this.player1bullets, this.player2bullets, this.playerMissileList);
                    Settings.saveFirstTime = false;
                }
                Utils.saveRepeat(fileName,this.players, this.enemies, this.bullets, this.player1bullets, this.player2bullets,
    ...
    public void gameRepeatSave() {
        if (Settings.saveFirstTime) {
            Utils.saveGame(fileName, players, enemies, bullets, player1bullets,
                player2bullets, playerMissileList);
            Settings.saveFirstTime = false;
        }
        Utils.saveRepeat(fileName,players, enemies, bullets, player1bullets, player2bullets,
            playerMissileList);
    }
    ...
    public List<Object> loadRepeat(String fileName, List<Object> gameState) throws IOException {
        ObjectInputStream in = null;
        try {
